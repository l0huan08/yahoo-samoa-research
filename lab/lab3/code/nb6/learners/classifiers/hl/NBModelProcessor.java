...
/**
 * NB-Plan6 - Parallel Naive Bayes
 * Can handle real values.
 * multiple NBModel that test instances.
 * receive model update information from NBAttributeStat.
 * send classification result to resultStream
 * 
 * @author hl
 *
...
public class NBModelProcessor implements Processor{
	...
	public Processor newProcessor(Processor p) {
		logger.info(" ^^^ NBModelProcessor newProcessor, id = {}",processorId);
	...
	public boolean process(ContentEvent event) {
		//logger.info(" NBModel process(), id={},event.key={}",this.processorId,event.getKey()); //|debug|
		...
		{
			InstancesContentEvent isce = (InstancesContentEvent)event;
			// only process testing instance, do not consider training data
			if (isce.isTesting())
			{
				boolean isLast =false;
				Instance[] insts = isce.getInstances();
				for (int i=0;i<insts.length-1;i++)
				{
					ResultContentEvent rce =predictInstance(insts[i], isce.getInstanceIndex()+i,isLast,isce.getEvaluationIndex());
					this.resultStream.put(rce);
				}
				...
				ResultContentEvent rce =predictInstance(insts[insts.length-1], isce.getInstanceIndex()+insts.length-1,isLast,isce.getEvaluationIndex());
				this.resultStream.put(rce);
			}
			//TODO: if NBModel need to consider training data, should add more code
			// but in NB-6 NBModel only test instances, and the training data 
			// is processed in NBAttributeStat
			
			// add by hl 2014.4.29 should always return false to avoid infinitely recreating the processors {{
			return false;
			// }}  add by hl 2014.4.29 
		}
		
		// add by hl 2014.4.18 updateModel when receiving NBAttStatEvent
		if (event instanceof NBAttStatEvent)
		{
			NBAttStatEvent attStatEvt = (NBAttStatEvent)event;
			updateModel(attStatEvt);
			
			// modify by hl 2014.4.29 should always return false to avoid infinitely recreating the processors {{
			//return true;
			// }}  modify by hl 2014.4.29 
		}
	...
	{
		return predictInstance(event.getInstance(),event.getInstanceIndex(),event.isLastEvent(),event.getEvaluationIndex());
	}
	 ...
	 * @return true if successful predicted
	 */
	private ResultContentEvent predictInstance(Instance inst, long instanceIndex, boolean isLast,int evaluationIndex)
	{
		if (inst==null)
		{
		...
		ResultContentEvent rce =  newResultContentEvent(votes,inst,instanceIndex,isLast,evaluationIndex);
	...
	private ResultContentEvent newResultContentEvent(double[] votes, InstanceContentEvent inEvent)
	{
		...
		ResultContentEvent rce = new ResultContentEvent(inEvent.getInstanceIndex(), inEvent.getInstance(), inEvent.getClassId(), votes, false); //avoid dead before all the instances were evaluated in S4 mode
	 ...
	 * @return ResultContentEvent to be sent into Evaluator PI or other destination PI.
	 */
	private ResultContentEvent newResultContentEvent(double[] votes, Instance inst, long instanceIndex, boolean isLast,int evaluationIndex)
	{
		//ResultContentEvent rce = new ResultContentEvent( instanceIndex, inst, (int)inst.classValue(), votes, isLast);
		ResultContentEvent rce = new ResultContentEvent( instanceIndex, inst, (int)inst.classValue(), votes, isLast);
		
		rce.setClassifierIndex(this.processorId);
		rce.setEvaluationIndex(evaluationIndex);
		return rce;
	}
	...
	private static double[] doNaiveBayesPrediction(Instance inst,
        ...
        for (int classIndex = 0; classIndex < votes.length; classIndex++) {
        	...
        	// TODO:  change nClass
        	// modify by hl 2014.4.18
        	// use the observer[iClass] which stores P(Cj) , instead of nClass
        	// it is sent by NBAttributeStat{{
        	//--------
        	//votes[classIndex] = observedClassDistribution.getValue(classIndex);
            //--------
        	int iClass = inst.classIndex();
        	votes[classIndex] = attributeObservers.get(iClass).probabilityOfAttributeValueGivenClass(classIndex, 0);
        	// }}
	...
	//add by hl 2014.4.18 updateModel
	private void updateModel(NBAttStatEvent attStatEvt)
	{
	// update attributeObservers from attribute iMin to attribute iMax
			int iMin = attStatEvt.getIMin();
			int iMax = attStatEvt.getIMax();
			AttributeClassObserver[] attObservers = attStatEvt.getAttributeObservers();
			for (int i=iMin;i<=iMax;i++)
			{
				this.attributeObservers.set(i, attObservers[i-iMin]);
			}
	}
